...
function equipInic(){ //CRIAR: EQUIPAMENTO INICIAL    
    ...
    txt.innerHTML += `<p>Você está armado com uma espada e vestido com uma armadura de 
        couro. Você tem uma mochila para guardar suas Provisões e quaisquer tesouros que 
        possa vir a encontrar.</p>`
    txt.innerHTML += `<p>Além disso, você pode levar uma garrafa de uma poção mágica, que 
        o ajudará na sua missão. Você poderá escolher qualquer uma das seguintes poções:</p>`
...
function dicaIntro(){ //DICA INTRODUTÓRIA
    ...
    txt.innerHTML += `<p>Tome nota e faça um mapa na medida em que vai explorando. Esse 
        mapa será inestimável em aventuras futuras e permitirá que você progrida 
        rapidamente através de áreas inexploradas.</p>`
    txt.innerHTML += `<p>Nem todas as áreas contêm tesouros. Muitas guardam apenas 
        armadilhas e criaturas com as quais você certamente entrará em conflito.</p>`
    txt.innerHTML += `<p>Há muitas passagens que resultam em buscas infrutíferas, e, 
    ...
    txt.innerHTML += `<p>O único caminho seguro envolve um mínimo de riscos, e qualquer 
        jogador, por mais que os índices iniciais sejam fracos, deverá ser capaz de chegar 
        ao destino com bastante facilidade.</p>`
    txt.innerHTML += `<p>Que a sorte dos deuses o acompanhe na aventura à sua frente!</p>`
...
function cxStatus(){ //GERA A CAIXA DE STATUS DO PERSONAGEM
    ...
    function abreInvent(){
        if(cmdBnt == 0){
            ...
            secStatus.style.height = (160 + tabEquip.offsetHeight) + 'px'
...
function batalha(nomeInimigo,habInimigo,energInimigo,chamaEv,fuga,turnoFuga){ //SISTEMA DE BATALHAS
    ...
    function ataque(){
        ...
        for(let i = 0; i <= time-1;i++){
            ...
            atkPlayer[i] = parseInt(rolAtkPlayer) + stats.habAtual + bonusAtk
